public class openHash {
    ...
    public openHash(int m) {
        this.m = m;
        keys = new String[m + 1]; // indices [1..m], index 0 unused
    ...
    public void insert(String key, String value) {
        ...
        int i = hash(key);
        int firstDeleted = -1; // track first tombstone slot
        int probes = 0;

        while (probes < m) {
            if (keys[i] == null) {
                // Empty slot — insert here (or reuse tombstone if found earlier)
                ...
                // Tombstone slot — remember it but keep probing for duplicate
    ...
    public String lookup(String key) {
        ...
        while (probes < m) {
            if (keys[i] == null && !deleted[i]) {
                return null; // empty (non-tombstone) slot means key absent
            }
    ...
    // Remove: return value and delete the entry (leave tombstone)
